...
bool LoginServer::recv_password(int data_conn, char* password) {
	...
	while(SSL_accept(ssl) <= 0) {
		if(sent_ready == false) {
			short ready = 0;
			ready = htons(ready);
			send(data_conn, &ready, sizeof(short), 0);
			sent_ready = true;
		}
	}
	...
	int errval = SSL_read(ssl, buffer, sizeof(buffer));
	if(errval <= 0) {
		fprintf(logfile, "%s Error receiving user password\n", (timestamp()).c_str());
		return false;
	}
...
std::string LoginServer::timestamp() {
...
void* LoginServer::get_in_addr(struct sockaddr *sa) {
