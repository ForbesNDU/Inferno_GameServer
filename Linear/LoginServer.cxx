...
LoginServer::LoginServer(uint16_t requested_port) {
	...
	// UDP socket to chat server 
        ...
        bool result = setup_UDP_client_connection(hostname, port, &chat_sock, &chat_addr, &chat_addrlen);
        if(!result) {
	...
	// UDP socket to match server
        port = "30000";
        result = setup_UDP_client_connection(hostname, port, &match_sock, &match_addr, &match_addrlen);
        if(!result) {
